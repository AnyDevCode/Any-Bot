...
module.exports = class SetModChannelsCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      description: oneLine`
        ...
        and Any Bot will only respond to members with permission to use those commands.
  ...
  async run(message, args) {
    ...
    const modChannelIds = await message.client.mongodb.settings.selectModChannelIds(message.guild.id);
    let oldModChannels = [];
    if (modChannelIds) {
      if (modChannelIds.includes(" ")){
        ...
        oldModChannels = [message.guild.channels.cache.get(modChannelIds)];
        oldModChannels = trimArray(oldModChannels).join(' ');
      }
    }
    ...
    if (args.length === 0) {
      await message.client.mongodb.settings.updateModChannelIds("", message.guild.id);
    ...
    await message.client.mongodb.settings.updateModChannelIds(channelIds, message.guild.id);
