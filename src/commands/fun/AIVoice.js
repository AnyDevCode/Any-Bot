...
module.exports = class aivoiceCommand extends Command {
  ...
  async run(message, args) {
    ...
    for (let i = 0; i < voices_raw.length; i++) {
      ...
      if (!voices_raw[i].is_private && !voices_raw[i].memberships.length) {
        //Put in the voices string
        //If the voice is not in the voices string
        if (!voices.includes(voice)) {
          voices += voices_raw[i].name + " \n";
        }
      }
    }
    if (!voices.includes(voice)) {
