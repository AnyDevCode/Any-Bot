...
let text = args.join(" "); //Argumentos para el logro.
 if (!text) return message.channel.send("Write some text for the achievement."); //Si no pone ningún argumento.
 if (text.length > 23) return message.channel.send("The achievement can only contain up to 23 characters."); //Si pone más de 23 carácteres en el argumento.
 if (text.length < 2) return message.channel.send("The achievement must contain more than 2 characters."); //Si pone menos de dos carácteres en el argumento.
