...
module.exports = class BotInfoCommand extends Command {
  ...
  async run(message) {
    const botOwner = "MDC#0001";
    const prefix = await message.client.mongodb.settings.selectPrefix(
      message.guild.id
    );
    const tech = stripIndent`
      ...
      Database    :: MongoDB with Mongoose v${require("mongoose/package.json").version}
      Shards      :: ${
        message.client.shard ? `${message.client.shard.count}` : "No Shards"
      }
    `;
