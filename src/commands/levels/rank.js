...
module.exports = class RankCommand extends Command {
  ...
  async run(message, args) {
    const member =  this.getMemberFromMention(message, args[0]) ||
      ...
      message.member;
    const xp = message.client.db.users.selectXP.pluck().get(member.id, message.guild.id);
    const level = message.client.db.users.selectLevel.pluck().get(member.id, message.guild.id);
    let total_messages = message.client.db.users.selectTotalMessages.pluck().get(member.id, message.guild.id);
    let total_commands = message.client.db.users.selectTotalCommands.pluck().get(member.id, message.guild.id);
    let total_reactions = message.client.db.users.selectTotalReactions.pluck().get(member.id, message.guild.id);
    let total_voice = message.client.db.users.selectTotalVoice.pluck().get(member.id, message.guild.id);
    let total_pictures = message.client.db.users.selectTotalPictures.pluck().get(member.id, message.guild.id);
    const requiredXP = 50 * Math.pow(level, 2);
    const leaderboard = message.client.db.users.selectRank.all(message.guild.id);
    ...
    let moderator_role = message.client.db.settings.selectModRoleId.pluck().get(message.guild.id) || "";
    let admin_role = message.client.db.settings.selectAdminRoleId.pluck().get(message.guild.id) || "";
    ...
    if (total_messages == null) total_messages = 0
      ...
      if ((1000 > total_messages) && ( total_messages >= 100)) total_messages = "bronze"
      if ((2500 > total_messages) && ( total_messages >= 1000)) total_messages = "silver"
      if ((10000 > total_messages) && ( total_messages >= 2500)) total_messages = "gold"
    ...
    if (total_commands == null) total_commands = 0
      ...
      if ((100 > total_commands) && (total_commands >= 10)) total_commands = "bronze"
      if ((500 > total_commands) && (total_commands >= 100)) total_commands = "silver"
      if ((1000 > total_commands) && (total_commands >= 500)) total_commands = "gold"
    ...
    if (total_reactions == null) total_reactions = 0
      ...
      if ((50 > total_reactions) && (total_reactions >= 10)) total_reactions = "bronze"
      if ((150 > total_reactions) && (total_reactions >= 50)) total_reactions = "silver"
      if ((300 > total_reactions) && (total_reactions >= 150)) total_reactions = "gold"
    ...
    if (total_pictures == null) total_pictures = 0
      ...
      if ((50 > total_pictures) && (total_pictures >= 10)) total_pictures = "bronze"
      if ((100 > total_pictures) && (total_pictures >= 50)) total_pictures = "silver"
      if ((200 > total_pictures) && (total_pictures >= 100)) total_pictures = "gold"
