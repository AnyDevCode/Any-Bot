...
module.exports = class BanCommand extends Command {
  ...
  async run(message, args) {
    ...
    if (member === message.member)
      return await this.sendErrorMessage(message, 0, 'You cannot ban yourself'); 
    if (member.roles.highest.position >= message.member.roles.highest.position)
      return await this.sendErrorMessage(message, 0, 'You cannot ban someone with an equal or higher role');
    if (!member.bannable)
