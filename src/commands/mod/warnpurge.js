...
module.exports = class WarnPurgeCommand extends Command {
  ...
  async run(message, args) {
    ...
    const autoKick = await message.client.mongodb.settings.selectAutoKick(
      message.guild.id
    ); // Get warn # for auto kick
    const autoBan = await message.client.mongodb.settings.selectAutoBan(
      message.guild.id
    ); // Get warn # for auto kick
    ...
    const totalwarns =
      parseInt((await message.client.mongodb.warns.maxWarnId()) + 1) || 1;
