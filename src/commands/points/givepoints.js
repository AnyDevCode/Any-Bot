...
module.exports = class GivePointsCommand extends Command {
  ...
  async run(message, args) {
    ...
    if (!member) return await this.sendErrorMessage(message, 0, 'Please mention a user or provide a valid user ID');
    ...
    if (isNaN(amount) === true || !amount)
      return await this.sendErrorMessage(message, 0, 'Please provide a valid point count');
    if (amount < 0 || amount > points) return await this.sendErrorMessage(message, 0, stripIndent`
