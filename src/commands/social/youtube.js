...
module.exports = class YoutubeCommand extends Command {
  ...
  async run(message, args) {
    ...
    if (!videoName) return await this.sendErrorMessage(message, 0, 'Please provide a YouTube video name');
    ...
    let result = await search(videoName, searchOptions)
      .catch(async err => {
        ...
        return await this.sendErrorMessage(message, 1, 'Please try again in a few seconds', err.message);
      });
    result = result.results[0];
    if (!result) 
      return await this.sendErrorMessage(message, 0, 'Unable to find video, please provide a different YouTube video name');
