...
module.exports = class CountryCommand extends Command {
    constructor(client) {
        super(client, {
            ...
            examples: ['country Peru', 'country USA'],
        });
    }
    ...
    async run(message, args) {
        ...
        await axios.get(url).then(res => {
            ...
            for (let lang in data.languages) {
                languages.push(`${lang} (${data.languages[lang]})`);
            }
            let embed = new MessageEmbed()
                ...
                .addField('Calling Code', `\`${data.idd.root}${((data.name.common === "United States") ? '' : data.idd.suffixes[0])}\``, true)
                ...
                .addField('Language(s)', `\`${languages.join(', ')}\``, true)
                .addField('Borders', `\`${data.borders.join(', ')}\``, true)
                .addField('Latitude', `\`${data.capitalInfo.latlng[0]}\``, true)
                .addField('Longitude', `\`${data.capitalInfo.latlng[1]}\``, true)
            ...
            const linkrow = new MessageActionRow()
                .addComponents(
                    ...
                    .setEmoji("🗺"),
                    new MessageButton()
                    .setURL(data.maps.openStreetMaps)
                    .setLabel("OpenStreetMaps")
                    .setStyle("LINK")
                    .setEmoji("🗺"),
                )
