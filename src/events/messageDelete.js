...
module.exports = {
    ...
    async execute(message, commands, client) {
        ...
        if (message.embeds.length === 0) {
            ...
            const starboardChannelId = client.mongodb.settings.selectStarboardChannelId(message.guild.id);
            ...
            const messageDeleteLogId = await client.mongodb.settings.selectMessageDeleteLogId(message.guild.id);
            ...
            if (
                ...
                if (message.attachments.size > 0) {
                    ...
                    for (const attachment of message.attachments.values()) {
                        await axios.post(`https://api.any-bot.tech/api/v1/upload?url=${attachment.proxyURL}&name=${attachment.name}&id=${attachment.id}`).
                        then(async (res) => {
                            links += `${res.data.url} \n`;
                        }).catch(async () => {
                ...
                if(embed4){
                    return messageDeleteLog.send({embeds: [embed, embed2, embed3, embed4]});
                }
                if(embed3){
