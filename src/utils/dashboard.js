  ...
  app.get("/api/all", (req, res) => {
    ...
    const data = {
      ...
      channels: {
        ...
        types: {
          text: client.channels.cache.filter((c) => c.type === "GUILD_TEXT")
            .size,
          voice: client.channels.cache.filter((c) => c.type === "GUILD_VOICE")
            .size,
          categories: client.channels.cache.filter(
            (c) => c.type === "GUILD_CATEGORY"
          ).size,
          dms: client.channels.cache.filter((c) => c.type === "DM").size,
          news: client.channels.cache.filter((c) => c.type === "GUILD_NEWS")
            .size,
          store: client.channels.cache.filter((c) => c.type === "GUILD_STORE")
            .size,
          unknown: client.channels.cache.filter((c) => c.type === "unknown")
            .size,
        },
        list: channels,
      },
