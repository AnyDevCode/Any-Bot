...
export = {
  ...
  async updateRow(
    userID: any,
    userName: any,
    userDiscriminator: any,
    guildID: any,
    guildName: any,
    moderatorID: any,
    moderatorName: any,
    moderatorDiscriminator: any,
    reason: any,
    dateIssued: any,
    warnID: any
  ) {
  ...
  async updatesqlitetomongo(WarnsArray: any) {
  ...
  async selectRow(warn_id: any) {
    const warn = await Warn.find({ warnID: warn_id });
    return warn;
  },

  async selectUserWarns(user_id: any, guild_id: any) {
  ...
  async selectGuildWarns(guild_id: any) {
    const warns = await Warn.find({ guildID: guild_id });
    return warns;
  },

  async updateGuildName(guild_id: any, guild_name: any) {
    const warn = await Warn.findOneAndUpdate(
      { guildID: guild_id },
      { guildName: guild_name },
      { new: true }
  ...
  async warnsByUser(user_id: any, guild_id: any) {
    const warns = await Warn.find({
      userID: user_id,
      guildID: guild_id,
    });
  ...
  async deleteUserWarns(user_id: any, guild_id: any) {
  ...
  async selectUserWarnsCount(user_id: any, guild_id: any) {
    const warns = await Warn.find({
      userID: user_id,
      guildID: guild_id,
    });
  ...
  async createWarn(
    ...
    moderator_discriminator: any,
    reason: any,
    date_issued: string | number | Date,
    warn_id: any
  ) {
  ...
  async getWarnById(warn_id: any) {
    const warn = await Warn.find({ warnID: warn_id });
    return warn[0];
  },

  async deleteWarn(warn_id: any) {
